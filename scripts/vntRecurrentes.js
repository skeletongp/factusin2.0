...
window.addEventListener("load", () => {
  ...
  let options = {
    language: {
      decimal: "",
      emptyTable: "No se encontraron tickets",
      info: "Mostrando de _START_ a _END_ de _TOTAL_ tickets",
      infoEmpty: "Mostrando 0 de 0 tickets",
      infoFiltered: "(filtrado de _MAX_ tickets en total)",
      infoPostFix: "",
      thousands: ",",
  ...
  function getAllFacturas() {
    ...
    data.recurrentes().then((row) => {
      tbcuerpo.innerHTML = null;

      Object.values(row).map((recurrente) => {
        if (recurrente.FacturaId > 0) {
          ...
          td1.innerText = recurrente.Fecha;
          td2.innerText = recurrente.Cliente;
          td3.innerText = formatter.format(recurrente.Monto);
